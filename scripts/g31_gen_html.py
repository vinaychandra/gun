...
tex_file = "./doc/g31_prof_report.tex"
...
answer = []
for line in data :
	answer.append(line.replace("\\it","<it>").replace("\\rm","</it>"))
data = answer
...
#author fix
data = data.replace("\\author{","<mauthors>",1).replace("}","</mauthors><br>",1)

thefile = open(html,'w')
#for x in data :
thefile.write("%s\n" % data)
thefile.write("</document></body></html>")
thefile.close()